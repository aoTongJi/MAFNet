...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='/data/StereoDatasets/kitti',
                 image_set='training', year=2015, indices=None):
        super(KITTI, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI)
        assert os.path.exists(root)

        def resolve_year_root(base_root, selected_year):
            names = [f'KITTI_{selected_year}', str(selected_year)]
            candidates = [osp.join(base_root, name) for name in names] + [base_root]
            image_dir = 'colored_0' if selected_year == 2012 else 'image_2'
            for candidate in candidates:
                if osp.isdir(osp.join(candidate, image_set, image_dir)):
                    return candidate
            raise FileNotFoundError(
                f'Could not find KITTI {selected_year} under {base_root!r}; '
                f'tried {candidates}')

        if year == 2012:
            root_12 = resolve_year_root(root, 2012)
        ...
        if year == 2015:
            root_15 = resolve_year_root(root, 2015)
        ...
        if not (len(image1_list) == len(image2_list) == len(disp_list) > 0):
            raise RuntimeError(
                f'Invalid KITTI {year} file counts: left={len(image1_list)}, '
                f'right={len(image2_list)}, disparity={len(disp_list)}')

        if indices is not None:
            selected = list(indices)
            image1_list = [image1_list[index] for index in selected]
            image2_list = [image2_list[index] for index in selected]
            disp_list = [disp_list[index] for index in selected]
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if dataset_name == 'sceneflow':
            ...
            data_root = getattr(args, 'data_root', '/data/StereoDatasets/kitti')
            kitti12 = KITTI(aug_params, root=data_root, year=2012)
            logging.info(f"Adding {len(kitti12)} samples from KITTI 2012")
            kitti15 = KITTI(aug_params, root=data_root, year=2015)
